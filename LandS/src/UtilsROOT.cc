...
bool ConfigureModel(CountingModel *cms, double mass,  TString ifileContentStripped, int debug){
	...
	for(int c=0; c<nchannel; c++){
		...
		cout<<" JAD: nsigproc["<<c<<"] = "<<nsigproc[c]<<endl;
		cout<<" JAD: channelnames["<<c<<"] = "<<channelnames[c]<<endl;
		for(int ijk=0;ijk<sigbkgs.size();ijk++) {
		  cout<<" JAD: sigbkgs["<<ijk<<"] = "<<sigbkgs[ijk]<<endl;
		}
		cout<<" JAD: observeddata["<<c<<"]"<<observeddata[c]<<endl;
		for(int ijk=0;ijk<sigbkgs.size();ijk++) {
		  cout<<" JAD: tmpprocn["<<ijk<<"] = "<<tmpprocn[ijk]<<endl;
		}
		...
		for(int p=0; p<ntotprocesses; p++){
			...
			if(pdf==typeLogNormal||pdf==typeTruncatedGaussian){
				if(ss[1]=="shapeStat"){
				  ...
				  cout << "yes, here two" << endl;
				  cout << "binnumber["<<p<<"]-1 "<<binnumber[p]-1<<endl;
				  cout << "subprocess["<<p<<"] "<<subprocess[p]<<endl;
				  cout << "err " << err << endl;
				  cout << "errup " << errup << endl;
				  cout << "pdf " << pdf << endl;
				  cout << "indexcorrelation " << indexcorrelation << endl;
				  cout << "bUncIsFloatInFit " << bUncIsFloatInFit << endl;
