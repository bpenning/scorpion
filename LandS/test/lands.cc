...
int main(int argc, const char*argv[]){
	...
	if(calcsignificance==0){// calc limits
	  cout << "JAD: not calculating significances!" << endl;
		if(method == "Bayesian"){
			runBayesian();
		}else if(method == "Hybrid"){
		  cout<< "JAD: Hybrid" << endl;
			if(ExpectationHints == "Asymptotic"){
			  cout << "JAD: Asymptotic"<<endl;
			  ...
			  cout << "JAD: no expectation hints supplied"<<endl;
			...
			if(scanRs && vR_toEval.size()>0){
			  cout<< "JAD: if(scanRs && vR_toEval.size()>0)" << endl;
			...
			if(makeCLsBands>=2){
			  cout<<"JAD:  if(makeCLsBands>=2)"<<endl;
			...
			if(!bSkipM2lnQ && makeCLsBands==1){
			  cout<<"if(!bSkipM2lnQ && makeCLsBands==1)"<<endl;
				clsr95.DoingStatisticalBandsForCLs(vsb, vb);	
				double errs;
			...
			if(!bNotCalcCLssbb && !bSkipM2lnQ){
			  cout<<"if(!bNotCalcCLssbb && !bSkipM2lnQ)"<<endl;
			...
			if(bSaveM2lnQ && !bSkipM2lnQ){
			  cout<<"JAD:  if(bSaveM2lnQ && !bSkipM2lnQ)"<<endl;
			...
			cout<<"JAD: do we get here?"<<endl;
			//delete cms;
			for(int hello=0; hello<-1; hello++){
			  cout<<"JAD: wtf is this loop doing?"<<endl;
			...
			if(bPlots && !bSkipM2lnQ){
			  cout<<"JAD: if(bPlots && !bSkipM2lnQ)"<<endl;
			...
			if(debug>=10 && !bSkipM2lnQ) {
			  cout<<"if(debug>=10 && !bSkipM2lnQ)"<<endl;
			...
			if(bCalcObservedLimit){
			  cout<<"JAD: if(bCalcObservedLimit)"<<endl;
			  if(bQuickEstimateInitialLimit) { std::cout << "quickestimate?" << std::endl; }
			...
			if(doExpectation && sFileLimitsDistribution==""){
			  cout<<"if(doExpectation && sFileLimitsDistribution==)"<<endl;
