...
SSb8::SSb8(const std::string & name, 
...
std::vector<jlepton> SSb8::getleptons(const TClonesArray *ELEC, const TClonesArray *MUON, const float & pte, const float & etae, const float & ptm, const float & etam) {
...
std::vector<jjet> SSb8::getjets(const TClonesArray *JET, const float & pt, const float & eta) {
...
double SSb8::getht(const std::vector<jjet> & jets) {
  double HT=0.0;
...
double SSb8::getmet(const TClonesArray *ETMISS) {
...
unsigned int SSb8::getnbtags(const std::vector<jjet> & jets) {
...
void SSb8::Run(const TreeReader & treereader, const TreeReader & gentreereader, const double & weight) {
