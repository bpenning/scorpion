...
ZeroLepDOJ::ZeroLepDOJ(const std::string & name, 
...
ZeroLepDOJ::~ZeroLepDOJ() {}

//TSimpleArray<TRootJet> ZeroLepDOJ::SubArrayGoodJets(const TClonesArray *JET, const float & pt, const float & eta) {
//
...
//TSimpleArray<TRootElectron> ZeroLepDOJ::SubArrayEl(const TClonesArray *ELEC, const float & pt, const float & eta) {
...
//TSimpleArray<TRootMuon> ZeroLepDOJ::SubArrayMu(const TClonesArray *MUON, const float & pt, const float & eta) {
...
//TSimpleArray<TRootJet> ZeroLepDOJ::SubArrayBadJets(const TClonesArray *JET, const float & pt, const float & eta) {
//
//  TIter itJet((TCollection*)JET);
//  TRootJet *jet;
//  itJet.Reset();
...
//TSimpleArray<TRootETmis> ZeroLepDOJ::makeETM(const TClonesArray *ETMISS) {
...
void ZeroLepDOJ::Run(const TreeReader & treereader, const TreeReader & gentreereader, const double & weight) {
  ...
  double calo_met = getmet(treereader.ETMis());
  calomethist->Fill(calo_met, weight);


  std::vector<jlepton> electrons = getleptons(treereader.Elec(), treereader.Muon(), 10.0, 2.5, 10.0, 2.4, CHARGEBOTH, FLAVOURELECTRON);
  std::vector<jlepton> muons = getleptons(treereader.Elec(), treereader.Muon(), 10.0, 2.5, 10.0, 2.4, CHARGEBOTH, FLAVOURMUON);

  //std::vector<jlepton> leptons = electrons;
  //leptons.insert(leptons.end(), muons.begin(), muons.end());

  std::vector<jjet> goodjets = getjets(treereader.Jet(), 50.0, 3.0, false);
  std::vector<jjet> badjets = getjets(treereader.Jet(), 50.0, 3.0, true);

  if(goodjets.size() >= 2 && badjets.size() == 0 && electrons.size() == 0 && muons.size() == 0) {
    if(goodjets[0].Pt() > 100.0 && goodjets[1].Pt() > 100.0 && fabs(goodjets[0].Eta()) < 2.5) {
      if(calo_met > 100.0) {
	double alphat = getAlphaT(goodjets);
	double sumht = getht(goodjets);
	hthist->Fill(sumht, weight);
	athist->Fill(alphat, weight);
      }
    }
  }
